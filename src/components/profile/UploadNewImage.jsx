...
const UploadNewImage = ({ setUploadImage, setnewImageUpload, uploadImage }) => {
    ...
    const handleSaveImage = () => {
        setnewImageUpload(selectedImage);
    ...
    return (
        <>
            <div className='absolute min-h-screen top-0 start-0 w-full h-full'>
                <div onClick={() => setUploadImage(false)}
                    className='absolute bg-black opacity-[0.4] z-20 w-full min-h-full h-full top-0 start-0'>
                </div>
                <div className='container max-w-[1440px] xl:px-6 mx-auto px-0 sm:py-3 h-full flex justify-center items-end sm:items-center'>
                    <div className='max-w-[480px] !w-full bg-black-500 rounded-xl mt-14 relative z-30'>
                        ...
                        <div className='border-b-[1px] p-4 md:p-6 border-lightBlack'>
                            {selectedImage ? (
                                <Image className='rounded-xl w-full h-[142px]'
                                    src={selectedImage}
                                    alt='uploaded image'
                                    width={432} height={142}
                                />
                            ) : (
                                <Image className='rounded-xl w-full h-[142px]'
                            ...
                            <p className='py-4 leading-6 tracking-[0.16px] font-normalidad text-md font-normal text-grey-400'>
                                For best results, we recommend you upload your banners at 952px by 280px as JPEG files and must be under 5MB.
                            </p>
                            <label htmlFor='uploadImage' className='py-3 p-3 rounded-xl cursor-pointer border-[1px] border-lightBlack border-solid hover:border-green-yellow uppercase font-normalidad font-medium text-md flex gap-2 w-full justify-center items-center text-white leading-4 tracking-[0.16px]'>
                        ...
                        <div className='p-4 flex gap-3 w-full'>
                            ...
                            <button onClick={handleSaveImage} type='button' className='text-black uppercase font-normalidad font-medium text-md leading-4 tracking-[0.16px] bg-green-yellow hover:bg-white transition-all duration-300 w-full p-4 rounded-xl' >Upload and save</button>
                        </div>
                    </div>
                </div>
            </div>
