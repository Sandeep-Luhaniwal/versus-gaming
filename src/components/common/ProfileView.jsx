  ...
  return (
    <>
      ...
      <div className={`container max-w-[1440px] w-full mx-auto flex justify-end`}>
        <div className={`mt-4 max-w-[320px] relative z-20 w-full bg-black-500 border-b-[1px] border-lightBlack rounded-xl`}>
          <div className="flex flex-col p-4">
            ...
            <div className='p-2 bg-lightBlack rounded-xl mt-3 flex gap-2 items-center'>
              ...
              <p>
                <span className='leading-4 text-md tracking-[0.16px] font-normalidad font-medium text-white'>The Lorem ipsum team</span>
                <span className='pt-1 leading-4 text-md tracking-[0.14px] font-normalidad font-normal text-green-yellow flex gap-1 items-center'><TeamIcon fill="#D4FF27" /> 5 players</span>
              </p>
            </div>
            <div className='py-4'>
              {loginData.map((data, index) => {
                return (
                  <div key={index} className='py-3'>
                    <p className='inline-block cursor-pointer group hover:text-white transition-all duration-300 font-normalidad text-md font-normal leading-4 tracking-[0.16px] text-black-400'><span className='flex items-center gap-2'>{data.icon}{data.title}</span></p>
                  </div>
                )
              })}
